...
class BookRecord:
    ...
    def to_payload(self, include_language: bool = False) -> dict:
        payload = {
            "id": self.id,
            "source": self.source or "gutenberg",
            "title": self.title,
            "author": self.author,
            "cover": self.cover or "",
        }
        if self.legacy_id:
            payload["legacyId"] = self.legacy_id
        ...
        for entry in author_dir.iterdir():
            ...
            for book in data.get("books") or []:
                ...
                record = BookRecord(
                    ...
                    source=clean_text(book.get("source") or "gutenberg") or "gutenberg",
                    legacy_id=clean_text(book.get("legacyId") or ""),
                    language=language,
                )
                existing = by_id.get(book_id)
                if existing and existing.source == "gutenberg" and record.source != "gutenberg":
                    continue
                by_id[book_id] = record
        return by_id
