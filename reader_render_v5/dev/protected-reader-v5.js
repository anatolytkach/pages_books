...
const ARTIFACT_ROOT_PREFIX = "/reader_render_v5/artifacts/protected-books/";
    ...
    <div class="v4-paginated-stage">
      <div class="v4-paginated-page-frame">
        <div class="v4-paginated-page">
          <div class="v4-page-content" data-v4-page-live="left"></div>
        </div>
      </div>
      <div class="v4-paginated-page-frame v4-paginated-page-frame-secondary">
        <div class="v4-paginated-page">
          <div class="v4-page-content" data-v4-page-live="right"></div>
        </div>
      </div>
      <aside class="v4-footnote-preview-shell" data-footnote-preview-shell hidden>
        <div class="v4-footnote-preview-body" data-footnote-preview-body>Select a footnote reference to preview its note content.</div>
...
function renderV3DerivedShell(book, semanticReadingFlow) {
  return renderPaginatedReadingSurface(book, semanticReadingFlow);
}
...
function mountPaginatedReadingSurface(book, semanticReadingFlow, footnoteSummary) {
  ...
  if (!shell) return;
  const liveContentLeft = shell.querySelector('[data-v4-page-live="left"]');
  const liveContentRight = shell.querySelector('[data-v4-page-live="right"]');
  ...
  if (!liveContentLeft || !liveContentRight || !measureContent || !prevButton || !nextButton || !counter) return;

  const units = buildPaginationUnits(book, semanticReadingFlow);
  let pages = [];
  let currentPageIndex = 0;
  let lastChapterTitle = "";
  const isDesktopSpread = () => {
    try {
      return !!(
  ...
  const getSpreadSize = () => (isDesktopSpread() ? 2 : 1);
  ...
  const renderPage = () => {
    const leftPageItems = pages[currentPageIndex] || [];
    const rightPageIndex = currentPageIndex + 1;
    const rightPageItems = isDesktopSpread() ? (pages[rightPageIndex] || []) : [];
    liveContentLeft.innerHTML = leftPageItems.map((item) => {
      const node = createPaginatedItemNode(item);
      return node.outerHTML;
    }).join("");
    liveContentRight.innerHTML = rightPageItems.map((item) => {
      const node = createPaginatedItemNode(item);
      return node.outerHTML;
    }).join("");
    liveContentRight.parentElement.parentElement.classList.toggle("hidden", !isDesktopSpread());
    counter.textContent = `${pages.length ? currentPageIndex + 1 : 0} / ${pages.length}`;
    prevButton.disabled = currentPageIndex <= 0;
    nextButton.disabled = currentPageIndex >= pages.length - getSpreadSize();
    prevButton.classList.toggle("disabled", currentPageIndex <= 0);
    nextButton.classList.toggle("disabled", currentPageIndex >= pages.length - getSpreadSize());
    lastChapterTitle = resolveShellChapterTitle(liveContentLeft, lastChapterTitle);
    if (isDesktopSpread()) {
      lastChapterTitle = resolveShellChapterTitle(liveContentRight, lastChapterTitle);
    }
    ...
    attachFootnotePreviewHandlers(footnoteSummary, shell);
    updateDiagnostics();
  };

  const repaginate = () => {
    const liveRect = liveContentLeft.getBoundingClientRect();
    measureContent.style.width = `${Math.max(Math.round(liveRect.width), 1)}px`;
    measureContent.style.minHeight = `${Math.max(Math.round(liveContentLeft.clientHeight), 1)}px`;
    measureContent.style.maxHeight = `${Math.max(Math.round(liveContentLeft.clientHeight), 1)}px`;
    pages = paginateUnitsIntoPages(units, measureContent);
    if (!pages.length) {
      pages = [[]];
    }
    currentPageIndex = Math.min(currentPageIndex, Math.max(0, pages.length - getSpreadSize()));
    renderPage();
  };
  ...
  const onPrev = () => goToPage(currentPageIndex - getSpreadSize());
  const onNext = () => goToPage(currentPageIndex + getSpreadSize());
...
function renderStatus(book, artifactRoot) {
  ...
  renderScreen(renderV3DerivedShell(book, semanticReadingFlow));
  const bookTitleNode = document.getElementById("book-title");
  if (bookTitleNode) {
    bookTitleNode.textContent = String(book && book.title || "Untitled");
  }
...
function renderError(message) {
  ...
  const bookTitleNode = document.getElementById("book-title");
  const chapterTitleNode = document.getElementById("chapter-title");
  const counterNode = document.getElementById("page-count");
  if (bookTitleNode) bookTitleNode.textContent = "V5 error";
  if (chapterTitleNode) chapterTitleNode.textContent = "";
  if (counterNode) counterNode.textContent = "0 / 0";
  renderScreen(
    `<div class="v4-card">
