...
function normalizeArtifactBookId(url, fallbackBookId = "") {
  ...
  const explicit =
    String(
      ...
      (hostConfig && hostConfig.protectedArtifactBookId) ||
      ""
    ).trim();
  if (isNumericId(explicit)) return explicit;
...
function resolveProtectedArtifactRoot(url, artifactBookId) {
  ...
  return artifactBookId
    ? `/books/protected-content-v4/${encodeURIComponent(artifactBookId)}`
    : "/books/protected-content-v4/19686";
}
...
export function parseProtectedIntegrationRoute(input = window.location.href) {
  ...
  const remoteMode = String(
    url.searchParams.get("readerRemoteMode") ||
      url.searchParams.get("protectedRemoteMode") ||
      ""
  )
    .trim()
    .toLowerCase();
  ...
  return {
    kind: "protected-reader-route-v2",
    url: url.toString(),
