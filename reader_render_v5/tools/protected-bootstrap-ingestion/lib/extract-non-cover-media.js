...
function isImageBlockClassName(value) {
  return /\bimage-block\b/i.test(String(value || "").trim());
}
...
function extractStandaloneParagraphImages({ xhtml, textHref, textFilePath }) {
  const out = [];
  const paragraphImagePattern = /<p\b([^>]*)>\s*<img\b([^>]*)\/?>\s*<\/p>/gi;
  let match = paragraphImagePattern.exec(xhtml);
  let index = 0;
  while (match) {
    const paragraphAttrs = parseAttributes(match[1]);
    if (isImageBlockClassName(paragraphAttrs.class || "")) {
      match = paragraphImagePattern.exec(xhtml);
      continue;
    }
    const attrs = parseAttributes(match[2]);
    if (/\binline-avatar\b/i.test(String(attrs.class || "").trim())) {
      match = paragraphImagePattern.exec(xhtml);
      continue;
    }
    const sourceHref = String(attrs.src || "").trim();
    if (sourceHref) {
      ...
      const assetPath = path.resolve(path.dirname(textFilePath), sourceHref);
      out.push({
        blockId: `${textHref.replace(/[^\w]+/g, "_")}-content-image-standalone-${String(index).padStart(4, "0")}`,
        sourceHref: textHref,
        mediaItems: [
          buildMediaItem({
            mediaId: `${textHref.replace(/[^\w]+/g, "_")}-content-media-standalone-${String(index).padStart(4, "0")}`,
            mediaRole: "content-image",
            sourceHref,
            resolvedHref: toResolvedHref(textHref, sourceHref),
            assetPath,
...
export function extractNonCoverMedia(inputRoot) {
  ...
  for (const file of files) {
    ...
    blocks.push(
      ...
      ...extractStandaloneParagraphImages({ xhtml, textHref, textFilePath }),
