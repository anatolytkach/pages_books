...
export async function exportProtectedAnnotationsToProductionNotes({
  ...
  for (const annotation of annotations) {
    ...
    } else if (annotation.metadata && annotation.metadata.compatQuote) {
      quote = String(annotation.metadata.compatQuote);
    }
    out.push({
...
export async function importProductionNotesToProtectedBundle({
  ...
  for (const rawNote of notes) {
    ...
    annotations.push({
      ...
      metadata: {
        productionCompat: cloneJson(note)
      }
    });
    if (note.comment) {
      annotations.push({
  ...
  return {
    bundle: createProtectedAnnotationBundle({
      bookId,
      userScope: "production-compat-import",
      annotations,
      metadata: {
