...
test("Unit: publish writes manual catalog and book-location entries", async (t) => {
  ...
  assert.equal(authorPayload.books[0].source, "manual");
  ...
  assert.equal(bookItem.source, "manual");
  assert.equal(bookItem.sourceBookId, "200123");
  assert.equal(bookItem.legacyId, "200123");

  const locationWrite = bucket.putCalls.find((entry) => entry.key === "api/book-locations/manual/23.json");
  assert.ok(locationWrite, "expected manual shard write");
  const locationPayload = JSON.parse(locationWrite.body);
  assert.equal(locationPayload.source, "manual");
