  ...
  */
   ...
   */
		...
		value: function located(location) {
			...
			var pageList = this.book && this.book.pageList;
			if (pageList && typeof pageList.pageFromCfi === "function") {
				var pageStart = pageList.pageFromCfi(start.mapping.start);
				var pageEnd = pageList.pageFromCfi(end.mapping.end);
  ...
  */
   ...
   */
		...
		value: function load(json) {
			...
			this.manifest = {};
			this.isJsonManifest = true;

			this.spine = json.spine.map(function (item, index) {
				var idref = item.idref || item.id || "spine-" + index;
				var manifestItem = {
					id: idref,
					href: item.href,
				...
				item.idref = idref;
				item.index = index;
				_this.manifest[idref] = manifestItem;
				return item;
			});

			json.resources.forEach(function (item, index) {
				var id = item.id || item.idref || item.href || "resource-" + index;
				var manifestItem = {
					id: id,
					href: item.href,
					type: item.type || "",
				...
				_this.manifest[id] = manifestItem;
			...
			// Mirror the typical OPF structure where <spine> is the 3rd element
			// child of <package> (metadata, manifest, spine), so generated CFIs
			// match the legacy EPUB contract expected by the reader.
			this.spineNodeIndex = 2;
